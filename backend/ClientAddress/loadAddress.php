...
function loadAdress($user_id, $type, $con){
  ...
  $stmt = null;
  if ($type == 'physical'){
    $stmt = $con->prepare("SELECT `cep`, `numero_clienteEnd`, `comp_endereco` FROM `tb_clienteEnd` WHERE `id_pf` = ?");
    $stmt->bindParam(1, $user_id);
    
  }else{
    $stmt = $con->prepare("SELECT `cep`, `numero_clienteEnd`, `comp_endereco` FROM `tb_clienteEnd` WHERE `id_pj` = ?");
    $stmt->bindParam(1, $user_id);
